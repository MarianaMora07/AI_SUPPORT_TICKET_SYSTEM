...
import type { SlaStatus } from '@/src/lib/sla';
import { getSlaLabel } from '@/src/lib/sla';
...
const slaColors: Record<SlaStatus, string> = {
  on_track:
    'border-emerald-300 bg-emerald-50 text-emerald-950 dark:border-emerald-700/60 dark:bg-emerald-900/50 dark:text-emerald-100',
  warning:
    'border-amber-300 bg-amber-50 text-amber-950 dark:border-amber-700/60 dark:bg-amber-900/50 dark:text-amber-100',
  breached:
    'border-red-300 bg-red-50 text-red-950 dark:border-red-700/60 dark:bg-red-900/50 dark:text-red-100',
  met: 'border-blue-300 bg-blue-50 text-blue-950 dark:border-blue-700/60 dark:bg-blue-900/50 dark:text-blue-100',
};
...
export function SlaBadge({ status }: { status: SlaStatus }) {
  return (
    <span className={`${badgeBase} ${slaColors[status]}`}>
      SLA: {getSlaLabel(status)}
    </span>
  );
}

export function AiPendingBadge({ label }: { label: string }) {
  return (
    <span className={`${badgeBase} border-amber-300 bg-amber-50 text-amber-950 dark:border-amber-700/60 dark:bg-amber-900/50 dark:text-amber-100`}>
      {label}
    </span>
  );
}
